...
const { exportarImagem } = require("../Utils/backup.js");

//Criando função que retornara a leitura de toda as imagens
const MostraImagem = (req, res) => {
    //Atribuindo a variavel string a função lerTodos
    let string = imagem.MonstraImagem()
...
//Criada função que retorna a leitura das imagens, filtando pelo id da pizza
const MostraImagemFiltradas = (req, res) => {
    //Atribuindo a variavel string a função lerTodos
    let string = imagem.MonstraImagemFiltradas(req.body)
    //executa uma consulta no banco de dados, que esta no return do 'lerTodos()', e em seguida passa dois paramentros, um de erro e outro de resultado
    conexao.query(string, (err, result) => {
        //Caso não haja erro, o programa teve uma consulta bem-sucedida.
        if (err == null) {
...
const CriarImagem = async (req, res) => {
    ...
    let string = imagem.CriarImagem(req.body)
